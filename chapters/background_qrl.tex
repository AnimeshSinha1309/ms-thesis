In this chapter, we introduce the basics of quantum computing and reinforcement learning the form the backdrop the work in the remainder of the thesis.
...
\begin{itemize}
    \item Grover Like - If a quantum algorithm succeeds with probability $p$, classically $1/p$ iterations are needed to succeed with constant certainty, whereas in the quantum setting using a technique called amplitude amplification, we can do the same in $O(1/\sqrt{p})$ iterations. These algoritms give a quadratic speedup over the classical.
    \item Shor's Like - Algorithms which
    \item Hamiltonian Simulation - Physical systems can be described by some hamiltonian, the eigenvalues of which are the achievable energy values of the system. 
\end{itemize}
...
Quantum Approximate Optimization Algorithms are a class of variational quantum algorithms which are used as a general purpose hammer to probabilistically solve combinatorial optimization problems.
...
\paragraph*{Algorithm}: The following steps are perfomed to execute QAOA for a given optimization problem.
\begin{enumerate}
